...
const apiKey = '?api_key=7f5c5275bb71a8b18b805f40d6c31b4f';
...
$(document).ready( () => {
    ...
    $('#movie-select').on('change', () => {
        if(isNaN($('#movie-select').val())){
			...
			var singleMovieUrl = `${apiBase}movie/${id}`;
            var movieData = getJsonInfo(`${singleMovieUrl}${apiKey}&language=en-US`);
            var relDate = new Date(movieData.release_date)
            var credits = getJsonInfo(`${singleMovieUrl}/credits${apiKey}&language=en-US`);
            var imgurl;
...
function getUpcomingMovies(){
    const apiUrl = `${apiBase}movie/upcoming${apiKey}&region=US`;
